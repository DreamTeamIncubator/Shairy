...
import { useAddCommentAnswerMutation, useGetCommentAnswersQuery } from "../api/comments";
...
const CommentItem = ({ comment: initialComment, postId, onClick, onAnswerClick}: CommentItemProps) => {
      ...
      <div className={s.avatarCommentWrapper}>
        ...
        <div className={s.commentDateLikeAnswer}>
          ...
          <div>
            ...
            <span className={s.like} onClick={() => onAnswerClick(comment.id, comment.from.username)}>Answer</span>
          </div>
          <div onClick={() => setOpenAnswers((prev) => !prev)}>
           {answer?.items && answer?.items?.length > 0 && (
          <span className={s.showAnswer}>
          {openAnswers ? `Hide answers: (${answer.totalCount})` : `Show answers: (${answer.totalCount})`}
          </span>
          )} 
         </div>
         {openAnswers && (
         ...
         <AnswerItem key={answer.id} answer={answer} onLike={() => console.log("Like answer")} />
        ))}
        </div>
        )}
