                ...
                {displayedPayments?.map(payment => {
                    return (
                        <div className={s.paymentsRow} key={payment.dateOfPayment}>
                            <div className={s.dateOfPayment}>{(new Date(payment.dateOfPayment).toLocaleDateString('ru-RU'))}</div>
                            <div className={s.endDate}>{(new Date(payment.endDateOfSubscription).toLocaleDateString('ru-RU'))}</div>
                            <div className={s.price}>{`$ ${payment.price}`}</div>
                            <div className={s.subscriptionType}>{payment.subscriptionType === 'DAY' ? '1 day' : payment.subscriptionType === 'MONTHLY' ? '1 month' : '7 days'}</div>
                            <div className={s.paymentType}>{payment.paymentType === 'PAYPAL' ? 'PayPal' : payment.paymentType === 'STRIPE' ? 'Stripe' : 'Credit Card'}</div>
                        </div>
                    )
                })}
            </div>
            <Pagination count={totalPages} onChange={handlePageChange} page={page} perPage={perPage}
